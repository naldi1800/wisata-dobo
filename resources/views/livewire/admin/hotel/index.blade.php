        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                @forelse($hotels as $hotel)
                    <tr class="{{ $hotel->trashed() ? 'opacity-50' : '' }} hover:bg-zinc-50 dark:hover:bg-zinc-800">
                        <td class="px-4 py-3">
                            @if($hotel->featured_image)
                                <img src="{{ asset('assets/images/hotels/' . $hotel->featured_image) }}" alt="{{ $hotel->name }}" class="h-16 w-16 object-cover rounded-lg" />
                            @else
                        ...
                        <td class="px-4 py-3">
                            <div>
                                <div class="font-medium text-zinc-900 dark:text-zinc-100">{{ $hotel->name }}</div>
                                <div class="text-sm text-zinc-500">{{ $hotel->slug }}</div>
                            </div>
                        </td>

                        <td class="px-4 py-3">
                            <div class="text-sm text-zinc-700 dark:text-zinc-300">{{ $hotel->address ?? '-' }}</div>
                        ...
                        <td class="px-4 py-3">
                            <span class="text-sm text-zinc-700 dark:text-zinc-300">{{ $hotel->hotel_class ?? '-' }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.hotels.edit', $hotel) }}" wire:navigate
                                ...
                                @if($hotel->is_active)
                                    <button wire:click="toggleActive({{ $hotel->id }})" class="text-zinc-600 hover:text-zinc-800">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if($hotel->trashed())
                                    <button wire:click="restore({{ $hotel->id }})" class="text-green-600 hover:text-green-800">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                        </svg>
                                    </button>
                                    <button wire:click="forceDelete({{ $hotel->id }})" class="text-red-600 hover:text-red-800">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                        </svg>
                                    </button>
                                @else
                                    <button wire:click="delete({{ $hotel->id }})" class="text-red-600 hover:text-red-800">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                        </svg>
                                    </button>
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-8 text-center">
                            <div class="text-zinc-500">No hotels found</div>
                            <div class="text-sm text-zinc-400">Get started by creating a new hotel.</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
